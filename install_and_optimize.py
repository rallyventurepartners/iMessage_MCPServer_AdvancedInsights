...
def configure_claude_desktop(logger):
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "update_claude_config.py"],
            check=True
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="iMessage Advanced Insights Installation and Optimization")
    parser.add_argument("--skip-indexing", action="store_true", help="Skip the database indexing step")
    parser.add_argument("--skip-install", action="store_true", help="Skip dependency installation")
    parser.add_argument("--claude-only", action="store_true", help="Only configure Claude Desktop integration")
    ...
    print("This script will install and optimize the iMessage Advanced Insights MCP Server.")
    ...
    if args.claude_only:
        ...
        if not args.skip_indexing and success:
            success = optimize_database(logger, args.custom_db, skip_prompt=False) and success
