...
async def parse_natural_language_time_period(query: str) -> Tuple[Optional[datetime], Optional[datetime]]:
    """
    ...
    if re.search(TIME_PATTERNS["this_week"], query, re.IGNORECASE):
        ...
        weekday = now.weekday()
        start_date = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=weekday)
        return start_date, end_date
...
async def extract_contact_info(query: str) -> Optional[str]:
    ...
    for pattern_name, pattern in CONTACT_PATTERNS.items():
        ...
        if match:
            ...
            return match.group(2) if match.group(2) else match.group(3) if match.group(3) else match.group(1)
    return None
    ...
    try:
        ...
        if analysis_type == "sentiment" and (contact_info or group_chat_info):
            ...
            active_contacts = [
                ...
                if contact.get("last_message_date") and 
                (not start_date or datetime.fromisoformat(contact["last_message_date"]) >= start_date) and
                (not end_date or datetime.fromisoformat(contact["last_message_date"]) <= end_date)
            ]
            
            active_group_chats = [
                chat for chat in group_chats
                if chat.get("last_message_date") and 
                (not start_date or datetime.fromisoformat(chat["last_message_date"]) >= start_date) and
                (not end_date or datetime.fromisoformat(chat["last_message_date"]) <= end_date)
            ]
            ...
            response["results"] = {
                "summary": {
                    ...
                    "top_contacts": sorted(active_contacts, key=lambda x: x.get("message_count", 0), reverse=True)[:5],
                    "top_group_chats": sorted(active_group_chats, key=lambda x: x.get("message_count", 0), reverse=True)[:5]
                }
            }
